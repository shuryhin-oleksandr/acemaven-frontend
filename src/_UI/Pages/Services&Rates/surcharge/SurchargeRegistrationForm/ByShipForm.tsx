...
const ByShipForm: React.FC<PropsType> = ({
  setShippingValue,
  shipping_modes,
  sea_carriers,
  ...props
  ...
  return (
    <FormWrap>
      <div style={{ display: "flex", width: "100%" }}>
        <GroupWrap>
          ...
          <Controller
            ...
            as={<SurchargeRateSelect label="Direction" options={carriers} />}
          />
          <Controller
            name="shipping_mode"
            control={props.control}
        ...
        <GroupWrap>
          ...
          >
            ...
            {props.ports && props.ports?.length > 0 && (
              <PortsList>
                {props.ports?.map((p: PortType) => (
                  <Port onClick={() => closePortsHandler(p)} key={p?.id}>
                    {p?.display_name}
                  </Port>
                ))}
              </PortsList>
          ...
          <SurchargesDates
            errors={{ from: props.errors.from, to: props.errors.to }}
            control={props.control}
      ...
      <>
        {!props.shippingValue ? (
          <UnderTitle>
            Please, complete the parameters of the surcharge for the value
            fields to appear
          </UnderTitle>
        ) : (
          <SeaCargoForm
