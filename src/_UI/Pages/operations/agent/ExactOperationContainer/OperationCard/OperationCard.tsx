      ...
      <CardContent>
        ...
        <ContentHeader>
          ...
          !operation_info.has_review ? (
            ...
            operation_info?.status !==
              AppOperationBookingStatusesType.CANCELLED_BY_SYSTEM &&
            operation_info?.status !==
              AppOperationBookingStatusesType.REJECTED && (
              <ActionsButtonsBlock
                agent_contact_name={props.agent_contact_name}
        ...
        <GeneralBlockContainer
          operation_info={operation_info}
          shipment={shipment ? shipment : null}
          company_type={company_type}
        />
        <ConfirmedDatesContainerBlock
          shipment={shipment ? shipment : null}
          operation_info={operation_info}
        />
        {(operation_info?.status ===
          ...
          operation_info?.status ===
            AppOperationBookingStatusesType.AWAITING_PAYMENT ||
          operation_info?.status ===
            AppOperationBookingStatusesType.SHIPMENT_IN_PROGRESS) && (
          <ShipmentTrackingBlock
            tracking={operation_info?.tracking}
            shipping_type={operation_info?.shipping_type}
            direction={
              operation_info?.freight_rate.origin.is_local ? "export" : "import"
            }
            origin_coordinates={
              operation_info?.freight_rate.origin.coordinates
                ? operation_info.freight_rate.origin.coordinates
                : null
            }
            destination_coordinates={
              operation_info?.freight_rate.destination.coordinates
                ? operation_info.freight_rate.destination.coordinates
                : null
            }
            company_type={company_type}
            ...
            departure={
              shipment?.actual_date_of_departure
                ? shipment?.actual_date_of_departure
                : shipment?.date_of_departure
                ? shipment?.date_of_departure
                : undefined
            }
            arrival={
              shipment?.actual_date_of_arrival
                ? shipment?.actual_date_of_arrival
          ...
          {company_type?.type === AppCompaniesTypes.AGENT
            ...
            : operation_info?.payment_due_by &&
              (operation_info?.status ===
                ...
                operation_info?.status ===
                  AppOperationBookingStatusesType.AWAITING_PAYMENT) && (
                <PaymentDueByForClient
                  payment_due_by={String(operation_info?.payment_due_by)}
        ...
        <ShipmentPartsBlock
          ...
          agent_info={{
            company: operation_info?.agent,
            contact_person: operation_info?.agent_contact_person,
          }}
          direction={operation_info?.tracking_initial?.direction}
        />
        <CargoBlock
